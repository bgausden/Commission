...
async function listBaselines(): Promise<void> {
  ...
  if (!(await fileExists(baselinesDir))) {
    ...
    console.log(
      "Create your first baseline with: npm run create-baseline -- <name>",
    );
    return;
  }
  ...
  console.log("");
  console.log(
    `Total: ${baselines.length} baseline${baselines.length !== 1 ? "s" : ""}`,
  );
  ...
  if (invalid.length > 0) {
    console.log("");
    console.log(
      `⚠️  ${invalid.length} baseline${invalid.length !== 1 ? "s have" : " has"} missing or invalid metadata.json`,
    );
  }
  ...
  console.log(
    "  npm run assemble-baseline -- --name <name> --month <m> --year <yyyy>",
  );
  console.log(
    "  npm run update-baseline -- <name>     Update existing baseline",
  );

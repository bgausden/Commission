 ...
 * ```
 */
export async function parseCommissionLog(
  filePath: string,
): Promise<StaffCommissionData[]> {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Parse revenue lines
    const generalRevenueMatch = line.match(
      /^General Services Revenue:\s*HK\$([\d,]+\.?\d*)$/,
    );
    ...
    const customRevenueMatch = line.match(
      /^(.+?)\s+Revenue:\s*HK\$([\d,]+\.?\d*)$/,
    );
    if (customRevenueMatch && !line.includes("General Services")) {
      const serviceName = customRevenueMatch[1].trim();
      currentStaff.customRateRevenues![serviceName] = parseAmount(
        customRevenueMatch[2],
      );
      continue;
    }

    // Parse commission lines
    const generalCommissionMatch = line.match(
      /^General Service Commission:\s*HK\$([\d,]+\.?\d*)$/,
    );
    if (generalCommissionMatch) {
      currentStaff.generalServiceCommission = parseAmount(
        generalCommissionMatch[1],
      );
      continue;
    }

    const customCommissionMatch = line.match(
      /^Custom Rate Service Commission:\s*HK\$([\d,]+\.?\d*)$/,
    );
    ...
    const productCommissionMatch = line.match(
      /^Product Commission:\s*HK\$([\d,]+\.?\d*)$/,
    );

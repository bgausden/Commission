...
async function createBaseline(options: CreateBaselineOptions): Promise<void> {
  ...
  try {
    ...
    await copyFile(
      staffHurdlePath,
      join(baselineDir, "config", "staffHurdle.json"),
    );
    ...
    const paymentsFile = outputFiles.find((f) =>
      f.includes("Talenox Payments"),
    );
    ...
    if (!contractorLog) {
      throw new Error("Contractor log not generated");
    }
    ...
    const metadata: BaselineMetadata = {
      ...
      staffIds,
      description:
        description || `Baseline for ${PAYROLL_MONTH}/${PAYROLL_YEAR} payroll`,
    };
...
- \`outputs/${contractorLog}\` - Contractor payments log
...
function parseFilename(filename: string): {
  ...
  if (!match) {
    throw new Error(
      `Cannot parse payroll month/year from filename: ${filename}`,
    );
  }
  return {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (args.length < 1) {
    console.error(
      "Usage: npx tsx scripts/createBaseline.ts <baseline-name> [commit-sha]",
    );
    console.error("");

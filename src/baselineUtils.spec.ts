...
describe('findOldestBaseline', () => {
  ...
  it('returns error when baselines directory does not exist', async () => {
    const result = await findOldestBaseline('/nonexistent/path/xyz-12345-does-not-exist');
  ...
  it('returns the only baseline when one exists', async () => {
    await writeBaseline(tempDir, 'jan-2025-baseline', '2025-01-15T10:00:00.000Z');
  ...
  it('returns the oldest baseline when multiple exist', async () => {
    await writeBaseline(tempDir, 'dec-2025-baseline', '2025-12-01T10:00:00.000Z');
    await writeBaseline(tempDir, 'jan-2025-baseline', '2025-01-15T10:00:00.000Z');
    await writeBaseline(tempDir, 'jun-2025-baseline', '2025-06-01T10:00:00.000Z');
...
async function writeBaseline(root: string, name: string, createdDate: string): Promise<void> {

...
export async function getTalenoxEmployees(): Promise<TTalenoxInfoStaffMap> {
  const getTalenoxEmployeesDebug = talenoxFunctionsDebug.extend('talenoxFunctionsDebug')
  ...
  result.forEach((staffInfo) => {
    if (staffInfo.employee_id === '') {
      getTalenoxEmployeesDebug('error: %s %O', 'employee_id is not set', staffInfo)
...
export async function createPayroll(
  ...
  staffMap.forEach((staffInfo, staffID) => {
    if (staffID === '') {
      ...
      throw new Error(`staffID is empty for ${staffInfo.first_name} ${staffInfo.last_name}`)
    }
  ...
  if (!response.ok) {
    ...
    response.text().then((text = 'No text from Talenox API call.') => createPayrollDebug('talenox response: %s', text))

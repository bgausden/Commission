...
if (process.env.DEFAULT_PAYMENTS_DIR && !fs.existsSync(process.env.DEFAULT_PAYMENTS_DIR)) {
  const message = `DEFAULT_PAYMENTS_DIR '${process.env.DEFAULT_PAYMENTS_DIR}' is not a valid path`
  errorLogger.error(message)
  throw new Error(message)
}

if (process.env.DEFAULT_DATA_DIR && !fs.existsSync(process.env.DEFAULT_DATA_DIR)) {
  const message = `DEFAULT_DATA_DIR '${process.env.DEFAULT_DATA_DIR}' is not a valid path`
  errorLogger.error(message)
  throw new Error(message)
}

if (process.env.DEFAULT_LOGS_DIR && !fs.existsSync(process.env.DEFAULT_LOGS_DIR)) {
